...
public class UIDemo_DataSort extends Golbal {
	public void ReadMe() {
	...
	static JFrame jframe = new JFrame(
			"JWebDW 数据排序演示: 请访问 http://webdw.vicp.net");
	...
	public static void setupUI() {
		...
		MyJButton cmdSort = new MyJButton("第二列排序", "cmdSaveText", targetControls,
				jPanel_Command);
		cmdSort.setBounds(new Rectangle(360, 10, 120, 20));
...
//		cmdUpdate.setBounds(new Rectangle(480, 10, 60, 20));
		...
		cmdSort.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			    ...
			    ui.Eval ("data.SetSort (" + "\"" + "#2 A" + "\"" + ")", iret);
			    if( iret.intvalue == -1){// Then
			        MsgBox( ui.errString, 0,"error");
			        return;
			    }else{
			        MsgBox( "设置排序条件为:" + "#2 A",0,"");
			    }
			    //time1 = Now()
			    ui.Eval( "data.Sort", iret);
			    
			    if( iret.intvalue == -1){// Then
			        MsgBox (ui.errString, 0, "sort error");
			        return;
			        //Exit Sub
			    }
